...
def test_calculate_reports_errors_with_a_line():
    result = _calc(asyncio.run(mcp.call_tool("calculate", {"expression": "1 +"})))
    assert result["error"].startswith("error (line 1):")
    assert result["value"] is None and result["exact"] is None and result["precision"] is None
...
def test_analyze_reports_malformed_expression_with_a_line_and_null_tree():
