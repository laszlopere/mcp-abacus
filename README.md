...
- **`solver`** — find the value(s) of one or more variables that drive an
  ...
  *golden-section search* (or *Brent parabolic* via `algorithm="brent-parabolic"`,
  ...
  `algorithm="ternary-search"`, or — for an extremum only — *Newton* via
  `algorithm="newton-optimise"`, which steps to the zero of the objective's own
  slope and lands a quadratic in one step; or, for a root, the sign-change bracketers
