...
def _solver_section() -> str:
    ...
    return "\n".join(
        [
            ...
            '              {"x": [0, 5], "y": [-4, 2]}. Needs a multivariate algorithm:',
            "              nelder-mead or powell (any objective), bfgs (extrema only).",
            ...
            "  ternary-search         single unknown, shrinks the 1-D bracket by THIRDS,",
            "                         dropping the outer third on the worse side. Same",
            "                         unimodal guarantee as golden-section, but it shrinks",
            "                         less per step AND spends two evaluations to do it, so",
            "                         ~2.4x the work; offered by name, not for speed.",
            "  newton-optimise        single unknown, EXTREMA ONLY (find-minimum /",
            "                         find-maximum): steps to the zero of the objective's own",
            "                         slope, x - g'/g'', fitting a parabola to the local value,",
            "                         slope and curvature and jumping to its vertex. Lands an",
            "                         exactly quadratic objective in ONE step. Refuses",
            "                         find-root: |expr| has a kink at the root.",
            ...
            "  brent-dekker           single unknown, find-root ONLY: the same bracket again,",
            "                         stepped by inverse-quadratic interpolation with a",
            "                         bisection fallback. Brent's ROOT method — note that bare",
            "                         'brent' names the brent-parabolic MINIMISER above.",
            "  chandrupatla           single unknown, find-root ONLY: the same interpolation as",
            "                         brent-dekker, admitted by a sharper test. Level with it on",
            "                         a simple root, markedly faster on a repeated one.",
            "  secant                 single unknown, find-root ONLY: steps to the zero of the",
            "                         chord through the last two points, bisecting the bracket",
            "                         whenever that leaves it. Leanest on a simple root; slowest",
            "                         of the family on a repeated one.",
            "  newton-raphson         single unknown, find-root ONLY: brackets NOTHING — steps",
            "                         along the expression's own slope, x - f(x)/f'(x), from the",
            "                         best point of a coarse scan, fenced into the bracket.",
            "                         Fastest on a smooth root, and (with halley) the only",
            "                         engine reaching a root that only TOUCHES zero.",
            "  halley                 single unknown, find-root ONLY: newton-raphson's step with",
            "                         the curvature added, 2ff'/(2f'**2 - ff'') — a tangent",
            "                         hyperbola rather than a tangent line. Converges cubically",
            "                         for the same five evaluations per step, falling back to the",
            "                         newton step wherever the curvature term stops being small.",
            "  nelder-mead            n unknowns, walks a downhill simplex; bounds-clamped to",
            "                         each bracket. Derivative-free, so it takes any objective.",
            "  powell                 n unknowns, derivative-free as well: each iteration",
            "                         minimises along every one of n directions in turn (a full",
            "                         bounded line search each, so an iteration here is worth n",
            "                         of the simplex's), then re-aims that direction set at the",
            "                         sweep's own net displacement — which follows a diagonal",
            "                         valley the simplex can only feel out. Usually the fewest",
            "                         evaluations of the three on a smooth objective.",
            "  bfgs                   n unknowns, EXTREMA ONLY: quasi-Newton gradient descent —",
            "                         builds a curvature model from gradients (no Hessian) and",
            "                         line-searches down it, box-clamped. Far fewer iterations",
            "                         than nelder-mead on a smooth bowl. Refuses find-root",
            "                         (|expr| has a kink at the root).",
            "",
...
def _fit_section() -> str:
    ...
    return "\n".join(
        [
            ...
            "generalized hyperbolic 1/(a*x**2 + b*x + c) by a reciprocal-quadratic (1/y = a*x**2 +",
            "b*x + c, the coefficients read directly), the Lorentzian peak a/(1 + ((x-b)/c)**2) by",
            "the same reciprocal-quadratic with the peak/centre/width recovered (the gaussian's",
            "symmetric-peak counterpart); the",
            ...
            *forms,
            ...
            "generalized hyperbolic and Lorentzian y != 0); the gaussian",
            "also needs bell-shaped, downward-parabola data, the Weibull a positive plot slope",
            "(shape b > 0), the Lorentzian an upward parabola in 1/y and a positive peak; rational",
            "mode also refuses the irrational logs/roots/sines/exp of the power, exponential,",
            "exp-reciprocal, Hoerl, Weibull, logistic, logarithmic, square-root, sinusoid,",
            "gaussian and Lorentzian forms (the",
            "saturation, hyperbolic, Laurent and generalized hyperbolic, being pure reciprocals/",
            "polynomials, stay exact in rational); and a polynomial needs",
